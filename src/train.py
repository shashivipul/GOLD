...
def main() -> None:
    ...
    for epoch in range(1, args.epochs + 1):
        ...
        # Every target node serves as an anchor, matching the (1/n_T) sum_i
        # in eq 15 exactly. Candidate sets C_i are still sampled per anchor
        # (num_pos positives from N(i), num_neg uniform negatives).
        anchor_idx = torch.arange(g_t.num_nodes(), device=device)
